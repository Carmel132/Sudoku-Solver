...
namespace Sudoku_Solver
{
    struct Board // 3 x 3 x 3 x 3 (3 x 3 cell) grid
    {
        ...
        public bool Solved()
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                if (!CheckRow(i + 1) && !CheckColumn(i + 1)) { return false; }
            }
        ...
        public bool IsLegal(Tuple<int, int, int> coord, Value val)
        {
            ...
            var original = temp.Data[coord.Item1].Data[coord.Item2][coord.Item3].Val;

            temp.Data[coord.Item1].Data[coord.Item2][coord.Item3].Val = val.Val;

            bool retval = CheckRow(coord.Item1 + coord.Item2 + 1) && CheckColumn(coord.Item1 % 3 + coord.Item3 + 1) && Data[coord.Item1].NoDuplicates();
            temp.Data[coord.Item1].Data[coord.Item2][coord.Item3].Val = original;
            return retval;
        }
